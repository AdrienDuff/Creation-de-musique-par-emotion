...
%Et y de de taille m*nb_features_sortie
fprintf('\nFichier de données. Ce fichier au format .mat doit contenir une matrice X de taille m*nb_de_noeuds_en_entrée, ainsi qu''une matrice Y de taille m*nb_de_noeuds_en_sortie.\n')
...
while file_ok ~= true;
  ...
  if ((exist(data_file) ~= 0) | (exist(strcat(data_file,'.mat')) ~= 0)); % Si le fichier existe, on le charge
    ...
    if size(X,2) ~= taille_Couches(1);
      ...
      if size(y,2) ~= taille_Couches(end);
        fprintf('\nLe nombre de colonne de y (%d) ne correspond pas au nombre de neurones en sortie (%d)',size(y,2),taille_Couches(end))
      else
